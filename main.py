...
def _hf_patched_attention_forward_method(
    ...
    **kwargs
    ...
    """Patched attention forward method for HuggingFace models."""
    ...
    # Capture query states during lookahead generation
...
def _qtip_patched_attention_forward_method(
    ...
    **kwargs
    ...
    """Patched attention forward method for QTIP models."""
    batch_size, query_length, _ = hidden_states.size()
...
class SpeculativePrefillPipeline:
    """Pipeline for speculative prefilling with optional KV cache sharing."""
    
    SUPPORTED_GRANULARITIES = ["global", "layer", "head"]
    POSITION_BUFFER = 20
    ...
    def __init__(
        ...
        # Validate arguments
        if self.kv_sharing_granularity not in self.SUPPORTED_GRANULARITIES:
            raise ValueError(
                f"Unsupported kv_sharing_granularity: {self.kv_sharing_granularity}. "
                f"Must be one of {self.SUPPORTED_GRANULARITIES}."
            )
        
        if not self.share_kv_cache and self.kv_sharing_granularity in ["layer", "head"]:
            raise ValueError(
                f"kv_sharing_granularity='{self.kv_sharing_granularity}' requires share_kv_cache=True."
            )
    ...
    def _extract_eos_token_ids(self, eos_token_id: Union[int, List[int], None]) -> List[int]:
        """Extract EOS token IDs from config."""
    ...
    def _is_qtip_model(self, model_name: str) -> bool:
        """Check if model name indicates a QTIP model."""
    ...
    def _load_model(self, model_name: str) -> AutoModelForCausalLM:
        """Load a model, either standard HuggingFace or QTIP."""
    ...
    def _check_model_compatibility(self):
        """Check if speculator and base models are compatible for KV cache sharing."""
    ...
    def _patch_speculator(self):
        """Patch speculator model's attention layers to capture query states."""
        if self.speculator_model is None:
            return 0
        ...
        for i, layer in enumerate(self.speculator_model.model.layers):
            if hasattr(layer, 'self_attn'):
                ...
                # Check if it's a HuggingFace LlamaAttention
                ...
                # Check if it's a QTIP LlamaAttention
    ...
    def _compute_granular_qk_scores(
        self,
        speculator_prefill_cache_as_tuple: Tuple[Tuple[torch.Tensor, torch.Tensor], ...],
        prompt_length: int,
        batch_size: int
    ):
        """Compute QK attention scores at different granularities."""
        ...
        for layer_idx in range(num_spec_layers):
            ...
            # Get key states from prefill cache
            ...
            # Accumulate scores across lookahead steps
    ...
    def _get_sorted_top_k_indices(
        ...
        """Get sorted indices of top-k highest scoring positions."""
        if current_seq_len == 0:
            return torch.empty(0, dtype=torch.long, device=device)
        
        # Ensure we keep at least one token if possible
        if num_to_keep == 0 and current_seq_len > 0:
            num_to_keep = 1
        ...
        # If keeping all tokens, return all indices
    ...
    def _prune_kv_cache(
        ...
        """Prune KV cache based on QK scores at specified granularity."""
        ...
        if original_seq_len > 0 and num_tokens_to_keep_target == 0:
            num_tokens_to_keep_target = 1
        num_tokens_to_keep_target = min(num_tokens_to_keep_target, original_seq_len)
        
        pruned_kv_list: List[Tuple[torch.Tensor, torch.Tensor]] = []
        
        if granularity == "global":
            # Validate global scores
            if not (self.global_qk_scores is not None and 
                ...
                raise ValueError(
                    f"Global QK scores shape ({self.global_qk_scores.shape if self.global_qk_scores is not None else 'None'}) "
                    f"is unsuitable for 'global' pruning. Expected bsz={batch_size}, seq_len={original_seq_len}."
                )
            ...
            # Get indices to keep based on global scores
            global_scores_for_selection = self.global_qk_scores[0]  # Assuming batch_size 1
            sorted_indices = self._get_sorted_top_k_indices(
                global_scores_for_selection, num_tokens_to_keep_target, original_seq_len, device
            )
            
            # Prune each layer using the same global indices
            ...
            # Validate layer scores
            if not (self.layer_qk_scores and len(self.layer_qk_scores) == len(spec_cache_tuple)):
                raise ValueError(
                    f"Layer QK scores list length ({len(self.layer_qk_scores) if self.layer_qk_scores else 'None'}) "
                    f"mismatch for 'layer' pruning. Expected {len(spec_cache_tuple)} layers."
                )
            ...
            for layer_idx, (k_l_spec, v_l_spec) in enumerate(spec_cache_tuple):
                ...
                # Validate layer-specific scores
                if not (self.layer_qk_scores[layer_idx].shape[0] == batch_size and 
                        self.layer_qk_scores[layer_idx].shape[-1] == current_layer_seq_len):
                    raise ValueError(
                        f"Layer {layer_idx} QK scores shape ({self.layer_qk_scores[layer_idx].shape}) is unsuitable. "
                        f"Expected bsz={batch_size}, seq_len={current_layer_seq_len}."
                    )
                
                if self.layer_qk_scores[layer_idx].sum().item() == 0 and num_tokens_to_keep_target < current_layer_seq_len:
                    raise ValueError(
                        f"Layer {layer_idx} QK scores are all zero, cannot perform meaningful pruning when prompt_keep_percentage < 1.0."
                    )
                
                # Get indices to keep for this layer
                layer_scores = self.layer_qk_scores[layer_idx][0]  # Assuming batch_size 1
                sorted_indices = self._get_sorted_top_k_indices(
                    layer_scores, num_tokens_to_keep_target, current_layer_seq_len, device
                )
            ...
            # Validate head scores
            if not (self.head_qk_scores and len(self.head_qk_scores) == len(spec_cache_tuple)):
                raise ValueError(
                    f"Head QK scores list length ({len(self.head_qk_scores) if self.head_qk_scores else 'None'}) "
                    f"mismatch for 'head' pruning. Expected {len(spec_cache_tuple)} layers."
                )
            
            # Prune each head independently
            for layer_idx, (k_l_spec, v_l_spec) in enumerate(spec_cache_tuple):
                current_layer_seq_len = k_l_spec.shape[2]
                ...
                if not (self.head_qk_scores[layer_idx].shape[0] == batch_size and 
                    ...
                    raise ValueError(
                        f"Layer {layer_idx} Head QK scores shape ({self.head_qk_scores[layer_idx].shape}) is unsuitable. "
                        f"Expected bsz={batch_size}, num_kv_heads={num_kv_heads_cache}, seq_len={current_layer_seq_len}."
                    )
                
                if self.head_qk_scores[layer_idx].sum().item() == 0 and num_tokens_to_keep_target < current_layer_seq_len:
                    raise ValueError(
                        f"Layer {layer_idx} Head QK scores are all zero, cannot perform meaningful pruning when prompt_keep_percentage < 1.0."
                    )
                ...
                for head_idx in range(num_kv_heads_cache):
                    ...
                    head_seq_len = k_lh_spec.shape[1]
                    ...
                    # Get indices to keep for this head
                    head_scores = self.head_qk_scores[layer_idx][0, head_idx, :]  # Assuming batch_size 1
                    sorted_indices_head = self._get_sorted_top_k_indices(
                        head_scores, num_tokens_to_keep_target, head_seq_len, device
                    )
        ...
        # Calculate final number of kept tokens
        final_kept_tokens = 0
    ...
    def run(
        ...
        # Initialize variables for speculator processing
        ...
        # Run speculator prefill if available
        if self.speculator_model is not None:
        ...
        # Generate lookahead tokens with speculator
        ...
        if (self.speculator_model is not None and 
            ...
            # Ensure we have a DynamicCache for lookahead
            if isinstance(current_spec_cache_lookahead, tuple):
                current_spec_cache_lookahead = DynamicCache.from_legacy_cache(current_spec_cache_lookahead)
            
            # Generate lookahead tokens
        ...
        # Compute QK scores if we generated lookahead tokens
        ...
        # Run base model prefill/knockout
        ...
        if self.speculator_model is None:
            # No speculator: standard base model prefill
            ...
            # Speculator used with KV cache sharing
            knockout_past_kv_dynamic: DynamicCache = DynamicCache()
            n_tokens_in_knockout_cache = 0
            
            if speculator_prefill_cache_as_tuple is not None and num_lookahead_steps > 0:
                # Prune KV cache based on QK scores
                ...
                # No cache to prune/share
                n_tokens_in_knockout_cache = 0
            
            # Run knockout pass with last token
            ...
            # Speculator used but not sharing KV cache (selective prefill)
            ...
            if (self.speculator_model is not None and 
                ...
                if self.global_qk_scores.sum().item() != 0 or prompt_keep_percentage == 1.0:
                    ...
                    if prompt_length > 0 and num_tokens_to_keep_sel == 0:
                        num_tokens_to_keep_sel = 1
                    num_tokens_to_keep_sel = min(num_tokens_to_keep_sel, prompt_length)
                    
                    global_scores = self.global_qk_scores[0]  # Assuming batch_size 1
                    indices_for_selective_prefill = self._get_sorted_top_k_indices(
                        global_scores, num_tokens_to_keep_sel, prompt_length, self.device
                    )
            ...
            if can_do_selective_prefill and prompt_keep_percentage < 1.0:
                # Selective prefill
                ...
                with torch.no_grad():
                    selective_out = self.base_model(
                        ...
                        past_key_values=selective_prefill_base_cache,
                        use_cache=True,
                ...
                # Generate first token with knockout
                knockout_tokens = prompt_input_ids[:, -1:]
                knockout_pos_ids = torch.tensor([[prompt_length - 1]], device=self.device, dtype=torch.long)
                knockout_cache_pos_base = torch.tensor(
                    [base_model_cache_after_sel_prefill.get_seq_length(0) if base_model_cache_after_sel_prefill.key_cache else 0],
                    device=self.device,
                    dtype=torch.long
                )
                ...
                with torch.no_grad():
                    first_token_out = self.base_model(
                        knockout_tokens,
                        position_ids=knockout_pos_ids,
                        past_key_values=base_model_cache_after_sel_prefill,
                        use_cache=True,
        ...
        timing_info["base_prefill_or_knockout"] = base_model_first_token_time
        
        if self.speculator_model is not None:
            timing_info["base_ttft"] = speculation_prefill_time + speculation_decode_time + base_model_first_token_time
        else:
            timing_info["base_ttft"] = base_model_first_token_time
        ...
        if base_model_next_token_ids is not None and base_model_cache_after_prefill is not None:
            ...
            # Continue generation if not EOS
        ...
        # Clean up Llama3-specific prefixes
...
def main():
    ...
    parser.add_argument("--kv_sharing_granularity", type=str, default="global",
                        ...
                        help="Granularity for KV cache pruning when sharing. 'global', 'layer', 'head' use QK scores.")
    args = parser.parse_args()
    
    # Handle "none" string for speculator model
    if args.speculator_model_name and args.speculator_model_name.lower() == "none":
        args.speculator_model_name = None
    
    # Print QTIP availability
    if QTIP_AVAILABLE:
    ...
    for stage, duration in timing_info_run.items():
    ...
    if args.share_kv_cache and args.speculator_model_name:
        print(f"Sharing Granularity: {args.kv_sharing_granularity}")
