...
def compress(model, args): 
    ...
    logging.info(f"CLAA compress: layers={layers}, tsp_idx={args.tsp_idx}, max_capacity_prompt={args.max_capacity_prompt}, max_capacity_prompt_percentage={args.max_capacity_prompt_percentage}, tsp_len={args.tsp_len}, tsp_len_percentage={args.tsp_len_percentage}, min_layer_idx={args.min_layer_idx}")
...
class CLAACluster():
    def __init__(self, window_size=8, max_capacity_prompt=512, kernel_size=7, pooling='avgpool', tsp_layer=False, tsp_length=2048, max_capacity_prompt_percentage=None, tsp_len_percentage=None, last_n_layers=None, min_layer_idx=0):
    ...
    def reset(self, window_size=8, max_capacity_prompt=512, kernel_size=7, pooling='avgpool', tsp_layer=False, tsp_length=2048, max_capacity_prompt_percentage=None, tsp_len_percentage=None, last_n_layers=None, min_layer_idx=0):
        self.window_size = window_size
        self.max_capacity_prompt = max_capacity_prompt
        self.max_capacity_prompt_percentage = max_capacity_prompt_percentage
        if self.max_capacity_prompt_percentage is None:
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups, layer_idx, **kwargs):
        ...
        # KV compression - only compress if we're at or after min_layer_idx
        ...
        if self.tsp_layer and (q_len > tsp_length):
            # Z-score normalize each layer's scores before aggregation
            scores_tensor = torch.stack(_score_buffer, dim=1)  # [batch, n_layers, heads, seq]
            
            normalized_scores = []
            for layer_idx_inner in range(scores_tensor.shape[1]):
                layer_scores = scores_tensor[:, layer_idx_inner, :, :]  # [batch, heads, seq]
            ...
            final_token_importance, _ = flattened_scores.max(dim=1)  # [batch, seq]
            
            # Get top-k indices for TSP
